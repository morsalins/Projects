        ...
        public override bool canRotate()
        {
            ...
            if (nextRotation == 1)
            {
                return right + 2 <= Bucket.rightEnd
                    ...
                    && !Bucket.bucketGrid[bottom - Bucket.baseRow, (right + 2) - Bucket.baseCol];
            }
            else if (nextRotation == 2)
            {
                return bottom + 2 < Bucket.bottomEnd
                    && !Bucket.bucketGrid[(bottom + 1) - Bucket.baseRow, left - Bucket.baseCol]
                    && !Bucket.bucketGrid[(bottom + 2) - Bucket.baseRow, left - Bucket.baseCol];
            }
            else if (nextRotation == 3)
            {
                return left - 2 >= Bucket.leftEnd
                    && !Bucket.bucketGrid[top - Bucket.baseRow, (left - 1) - Bucket.baseCol]
                    && !Bucket.bucketGrid[top - Bucket.baseRow, (left - 2) - Bucket.baseCol];
            }
            else if (nextRotation == 0)
            {
                return top - 2 > Bucket.topEnd
                    && !Bucket.bucketGrid[(top - 1) - Bucket.baseRow, right - Bucket.baseCol]
                    && !Bucket.bucketGrid[(top - 2) - Bucket.baseRow, right - Bucket.baseCol];
            }
            return false;
        }
        ...
        public override bool canMoveLeft()
        {
            if (currentRotation == 0)
            {
                return left - 1 >= Bucket.leftEnd
                    ...
                    && !Bucket.bucketGrid[(top + 1) - Bucket.baseRow, left - Bucket.baseCol]
        ...
        public override bool canMoveDown(int steps)
        {
            ...
            else if (currentRotation == 1)
            {
                while (x <= steps && Continue)
                {
                    if (bottom + x < Bucket.bottomEnd
                        && !Bucket.bucketGrid[(bottom + x) - Bucket.baseRow, left - Bucket.baseCol]
                        && !Bucket.bucketGrid[(bottom + x) - Bucket.baseRow, (left + 1) - Bucket.baseCol]
                        && !Bucket.bucketGrid[(bottom + x) - Bucket.baseRow, right - Bucket.baseCol])
                    {
                        x++;
                    }
            ...
            else if (currentRotation == 3)
            {
                while (x <= steps && Continue)
                {
                    if (bottom + x < Bucket.bottomEnd
                        ...
                        && !Bucket.bucketGrid[(top + x) - Bucket.baseRow, (left + 1) - Bucket.baseCol]
                        && !Bucket.bucketGrid[(bottom + x) - Bucket.baseRow, right - Bucket.baseCol])
                    {
                        x++;
                    }
