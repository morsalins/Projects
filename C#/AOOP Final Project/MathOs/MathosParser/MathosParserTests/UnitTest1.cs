...
namespace MathosTest
{
    ...
    public class MathParserTest
    {
        ...
        [TestMethod]
        public void ComparisonOfNumericalAnswers()
        {
            var mp = new MathParser();

            // här har vi det riktiga svaret
            var rightValue = mp.Parse("3pi/2");
            var errorBound = 0.001M;


            var answerAlteratives = new string[] { "4.71", "(3/2)pi", "6pi/4" };

            // kollar igenom svar
            for (int i = 0; i < answerAlteratives.Length; i++)
            {
                if (CheckErrorBound(rightValue, mp.Parse(answerAlteratives[i])) > errorBound)
                {
                    Assert.Fail();
                }
            }
        ...
        public decimal CheckErrorBound(decimal theoretical, decimal actual)
        {
            return Abs(theoretical - actual) / theoretical;
        }
        public decimal Abs(decimal num)
        {
            if (num >= 0)
            {
                return num;
